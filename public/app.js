...
function enterApp() {
  ...
  dashboardNavItem.hidden = CURRENT_USER.department !== "management";
  ...
  document.querySelectorAll(".nav-item").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.getElementById("view-title").textContent =
        view === "board" ? "Release board" : view === "calendar" ? "Calendar" : view === "admin" ? "Pending accounts" : "Dashboard";
...
async function loadDashboard() {
  ...
  let d;
  try {
    d = await api("/api/dashboard-summary");
  } catch (err) {
    el.innerHTML = `<p class="empty-state">${err.message}</p>`;
    return;
  }

  const digitalRemaining = d.budgets.digital.budget - d.budgets.digital.spent;
  const offlineRemaining = d.budgets.offline.budget - d.budgets.offline.spent;

  const stageRows = STAGES.map((s) => {
    const count = d.stage_counts[s.key] || 0;
    const pct = d.total_titles > 0 ? Math.round((count / d.total_titles) * 100) : 0;
    return `
  ...
  const upcomingHtml = d.upcoming_releases.length
    ? d.upcoming_releases.map((t) => `
        <div class="calendar-row" data-id="${t.id}">
          <span class="calendar-date">${formatDate(t.target_release_date)}</span>
          <span class="calendar-title">${escapeHtml(t.name)}</span>
          <span class="calendar-stage-tag tag-${t.stage}">${STAGES.find((s) => s.key === t.stage)?.label}</span>
        </div>`).join("")
    : "<p class='empty-state'>No upcoming release dates.</p>";

  el.innerHTML = `
    <div class="metric-grid">
      <div class="metric-card"><div class="metric-label">Total titles</div><div class="metric-value">${d.total_titles}</div></div>
      <div class="metric-card"><div class="metric-label">Ad slots locked</div><div class="metric-value">${d.ad_deals.locked || 0}</div></div>
      <div class="metric-card"><div class="metric-label">Ad slots available</div><div class="metric-value">${d.ad_deals.available || 0}</div></div>
      <div class="metric-card"><div class="metric-label">Pending account approvals</div><div class="metric-value">${d.pending_approvals}</div></div>
    </div>

    <div class="dash-section">
      <h3>Titles by stage</h3>
      ${stageRows}
    </div>
    ...
    <div class="dash-section">
      ...
      <div class="metric-grid">
        <div class="metric-card"><div class="metric-label">Digital budget</div><div class="metric-value">${fmtMoney(d.budgets.digital.budget)}</div></div>
        <div class="metric-card"><div class="metric-label">Digital spent</div><div class="metric-value">${fmtMoney(d.budgets.digital.spent)}</div></div>
        <div class="metric-card"><div class="metric-label">Offline budget</div><div class="metric-value">${fmtMoney(d.budgets.offline.budget)}</div></div>
        <div class="metric-card"><div class="metric-label">Offline spent</div><div class="metric-value">${fmtMoney(d.budgets.offline.spent)}</div></div>
      </div>
      <p class="detail-desc">Digital remaining: ${fmtMoney(digitalRemaining)} · Offline remaining: ${fmtMoney(offlineRemaining)}</p>
    </div>
  ...
  el.querySelectorAll(".calendar-row").forEach((row) => {
    row.addEventListener("click", () => openDetail(row.dataset.id));
  });
}
